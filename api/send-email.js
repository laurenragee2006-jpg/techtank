export default async function handler(req, res) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#0e0e0e;color:#0F172A;border-radius:12px">
          <p style="font-family:Georgia,serif;font-size:20px;margin:0 0 4px">lu<span style="color:#61d3f0">cent</span></p>
          ...
          <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:18px;margin-bottom:12px">
            ...
            <p style="color:#61d3f0;font-family:monospace;font-size:13px;margin:0;word-break:break-all">${applyLink}</p>
          </div>
          <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:18px;margin-bottom:12px">
